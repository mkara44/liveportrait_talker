...
from src.utils.lp_render.helper import load_lip_array, calc_motion_multiplier, smooth
...
class LivePortraitRender:
    ...
    def __call__(self, batch):
        ...
        if ref_rendering_input_face is not None:
            ref_template_dct = self.make_ref_head_pose_template(ref_rendering_input_face.to(self.device))
        ...
        for i in tqdm(range(n_frames), "Rendering.."):
            ...
            R_new = R_s_i if ref_rendering_input_face is None else ref_template_dct["R"][i] #if batch["source_type"] == "video" else x_d_i_info["R"][i]
